...
class PartyIdentification(AbstractXMLFileState):
    ...
    """
    _mapping = dict()
    _elementTag: str = 'PartyIdentification'
    ...
    def define_mappings(self, tag: str, initiator: AbstractXMLFileState):
        ...
        if tag == self._invoiceElementTag:
            ...
            self._mapping['ID'] = (
                'cbc', 'accountingsupplierparty_partyidentificationid', 'Zorunlu (1)', False, True, True, '')
            self._mapping['schemeID'] = (
                '', 'accountingsupplierparty_partyidentification_schemeid', 'Zorunlu (1)', False, False, False, '')
        elif tag == self._despatchElementTag:
            # _mapping[tag] = (namespace, frappe_field, cardinality, start_event, has_attribs, end_event)
            self._mapping['ID'] = ('cbc', '', 'Zorunlu (1)', False, True, True, '')
