    ...
    def process_elementasdict(self, element: Element, cbcnamespace: str, cacnamespace: str) -> dict:
        ...
        if invoicedquantity is not None:
            if invoicedquantity.text is not None:
                frappedata['invoicedquantity'] = invoicedquantity.text.strip()
                frappedata['invoicedquantityunitcode'] = invoicedquantity.attrib.get('unitCode').strip()
        ...
        if lineextensionamount is not None:
            if lineextensionamount.text is not None:
                frappedata['lineextensionamount'] = lineextensionamount.text.strip()
                frappedata['lineextensionamountcurrencyid'] = lineextensionamount.attrib.get('currencyID').strip()
        # ['Note'] = ('cbc', 'note', 'Seçimli (0...1)')
        note_ = element.find('./' + cbcnamespace + 'Note')
        if note_ is not None:
            if note_.text is not None:
                frappedata['note'] = note_.text.strip()
        ...
        if priceamount is not None:
            if priceamount.text is not None:
                frappedata['priceamount'] = priceamount.text.strip()
                frappedata['priceamountcurrencyid'] = priceamount.attrib.get('currencyID').strip()
        ...
        if taxtotal_ is not None:
            ...
            if taxamount_ is not None:
                if taxamount_.text != '':
                    frappedata['taxamount'] = taxamount_.text.strip()
                    frappedata['taxamountcurrencyid'] = taxamount_.attrib.get('currencyID')
